...
class Controller:
    ...
    def handle_detect(
        ...
        try:
            ...
            rs_map = analyzer.analyze(filepath)
            rs_confidence = analyzer.stego_confidence(rs_map)
            print("RS confidence: " + str(rs_confidence))

            # Obtain RS Analysis heatmap(highlights suspicious areas)
            rs_heatmap_fig = analyzer.make_heatmap_figure(rs_map)

            img_grayscale = cv2.imread(filepath, cv2.IMREAD_GRAYSCALE)
            rs_overlay_fig = analyzer.make_old_style_overlay_figure(img_grayscale, rs_map)
